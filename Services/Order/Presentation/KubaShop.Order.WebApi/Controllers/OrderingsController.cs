...
namespace KubaShop.Order.WebApi.Controllers
{
    ...
    public class OrderingsController : ControllerBase
    {
        ...
        {
            var values = await _mediator.Send(new GetOrderingQuery()); //Send=>Mediator dizayn patternde handle metotlarına erişim sağlamak için kullanılır.Send metotdunun için IRequisti miras alan sınıf çağırılır.
            return Ok(values);
        }
        [HttpGet("{id}")]
